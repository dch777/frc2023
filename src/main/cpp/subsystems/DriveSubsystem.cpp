...
void DriveSubsystem::Periodic() {
  frc::SmartDashboard::PutNumber("Heading", m_ahrs->GetYaw());
}
...
void DriveSubsystem::Drive(units::meters_per_second_t xSpeed, units::meters_per_second_t ySpeed, units::radians_per_second_t rot, bool fieldCentric) {
  ...
  if (fieldCentric) {
    speeds = frc::ChassisSpeeds::FromFieldRelativeSpeeds(xSpeed, ySpeed, rot, (units::degree_t) (m_ahrs->GetYaw() + 180));
  } else {
